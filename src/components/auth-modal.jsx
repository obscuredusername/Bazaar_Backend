  ...
  const handleGoogleSignIn = () => {
    ...
    if (window.google && googleInitialized) {
      try {
        ...
        // Create a unique context ID for each prompt to prevent suppression
        const contextId = `google-auth-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`
        ...
        const handleManualOAuth = () => {
          ...
          // Build the OAuth URL with state parameter containing auth mode
          const state = encodeURIComponent(JSON.stringify({ authMode }))
          const oauthUrl = `https://accounts.google.com/o/oauth2/auth?client_id=${clientId}&redirect_uri=${redirectUri}&response_type=code&scope=${scope}&prompt=select_account&state=${state}`
          ...
          const handleMessage = (event) => {
            ...
            if (event.data && event.data.type === 'google_auth_callback') {
              if (event.data.credential) {
                // Process the credential - pass the authMode as well
                const responseWithMode = {
                  ...event.data,
                  authMode: authMode // Add authMode to the response
                }
                handleGoogleResponse(responseWithMode)
        ...
        try {
          ...
          setTimeout(() => {
            try {
              ...
              }, { context: contextId }) // Use a unique context ID each time
            } catch (err) {
